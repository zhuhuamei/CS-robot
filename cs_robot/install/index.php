...
echo '<html>
<head>
<title>CS_robot在线客服系统 - 安装向导</title>
...
<tr>
<td class="title">CS_robot在线客服系统'.$CSrobotVersion.' - 安装向导</td>
</tr>
<tr>
...
if(defined('WELIVE')){
	echo '<font class=ohredb><b>CS_robot在线客服系统已经安装!</b></font><BR><BR>
...
$tableprefix     = isset($_POST['install']) ? trim($_POST['tableprefix'])     : 'csrobot_';
...
if(isset($_POST['install'])){
	...
	if(!isset($installerrors)){
		$SqlLines = @file('cs_robot.sql');
		if (!$SqlLines) {
			$installerrors[] = '无法加载数据文件: install/cs_robot.sql';
		} else {
			if(!$confirmprefix) {
			...
			if(!$tableprefix_err){
				...
				/* 替换表前缀 */
				$sql = preg_replace('/((TABLE|INTO|IF EXISTS) )cs_robot_/', '${1}' . $tableprefix, $sql);
				...
				DB_Query ("INSERT INTO " . $tableprefix . "user VALUES (NULL, 2, 1, 'mszhang', 1, '".md5($password)."', 1, 0, '张纤薇', 'Ms.Zhang', '姓名: 张纤薇', 'Name: Ms. Zhang', '广告', 'Adv.', 0) ");
				DB_Query ("INSERT INTO " . $tableprefix . "user VALUES (NULL, 2, 2, 'msbao', 1, '".md5($password)."', 1, 0, '包慧芳', 'Ms.Bao', '姓名: 包慧芳', 'Name: Ms. Bao', '广告', 'Adv.', 0) ");
				DB_Query ("INSERT INTO " . $tableprefix . "user VALUES (NULL, 3, 3, 'mszhu', 1, '".md5($password)."', 1, 0, '祝华梅', 'Ms.Zhu', '姓名: 祝华梅', 'Name: Mr. Zhu', '广告', 'Adv.', 0) ");
				...
				$contents = preg_replace("/[$]_CFG\['cAppVersion'\]\s*\=\s*[\"'].*?[\"'];/is", "\$_CFG['cAppVersion'] = '$CSrobotVersion';", $contents);
...
?>";
				...
				echo '<font class=ohblueb>恭喜: 您的CS_robot在线客服系统 安装成功!</font><br /><br />请在删除CS_robot安装目录(./install/)后继续!
					<br /><br />
...
if(!isset($_POST['install']) OR isset($installerrors) OR $tableprefix_err){
	...
	<tr>
	<td valign="top" align="right"><u>CS_robot ' .$CSrobotVersion. ' 简体中文版(UTF-8)</u></td>
	</tr>  
	</table>
	<br />
	<b>1) 填写CS_robot数据库连接信息:</b><br /><br />
	...
	<br /><br /><center><input type="submit" name="install" value="安装 CS_robot" /></center>
